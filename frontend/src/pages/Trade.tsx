...
function Trade() {
  ...
  const [activeTab, setActiveTab] = useState<'create' | 'myOffers' | 'browse'>('create');
  ...
  const loadData = async () => {
    try {
      ...
      setMyOffers(offers.filter(o => o.isActive));

      
      const active = await tradeService.getActiveOffers();
      setAllOffers(active.filter(o => o.creator.toLowerCase() !== walletAddress.toLowerCase()));

      console.log(' Données chargées:', nfts.length, 'NFTs,', offers.length, 'offres');
    } catch (error) {
  ...
  const handleCreateOffer = async () => {
    if (!selectedMyNFT || !requestedNFTId) {
      alert('Veuillez sélectionner un NFT à proposer et entrer l\'ID du NFT demandé');
      return;
    }
    ...
    try {
      ...
      const offerId = await tradeService.createOffer(selectedMyNFT, parseInt(requestedNFTId));

      alert(`Offre créée avec succès ! ID: ${offerId}`);
  ...
  const handleAcceptOffer = async (offer: TradeOffer) => {
    if (!confirm(`Voulez-vous échanger votre NFT #${offer.requestedNFT} contre le NFT #${offer.creatorNFT} ?`)) {
      return;
    }
                    ...
                    <>
                      <div className="form-section">
                        ...
                        <div className="nft-grid">
                          {myNFTs.map(nft => (
                            <div
                              key={nft.tokenId}
                              className={`nft-card ${selectedMyNFT === nft.tokenId ? 'selected' : ''}`}
                            ...
                            >
                              ...
                              <div className="nft-info">
                                <h4>{nft.name}</h4>
                                <span className="nft-rarity" style={{ color: getRarityColor(nft.rewardTypeName) }}>
                                  {nft.rewardTypeName}
                                </span>
                      ...
                      <div className="form-section">
                        ...
                        <p className="help-text">
                           Trouvez l'ID du NFT dans l'inventaire du joueur ou dans l'onglet "Parcourir"
                        </p>
                      </div>
                      ...
                      <button
                        ...
                        disabled={loading || !selectedMyNFT || !requestedNFTId}
                      >
                          ...
                          <div key={offer.offerId} className="offer-card">
                            <div className="offer-header">
                              ...
                              <span className="offer-date">
                                {new Date(offer.createdAt * 1000).toLocaleDateString('fr-FR')}
                              </span>
                            </div>
                            <div className="offer-body">
                              <div className="offer-nft">
                                ...
                                {myNFT ? (
                                  <div className="nft-preview">
                                    ...
                                    <div>
                                      <div>{myNFT.name}</div>
                                      <div style={{ color: getRarityColor(myNFT.rewardTypeName) }}>
                                        {myNFT.rewardTypeName}
                                      </div>
                                      <div className="token-id">#{offer.creatorNFT}</div>
                                    </div>
                                  </div>
                                ) : (
                              ...
                              <div className="offer-nft">
                                ...
                                <div className="nft-preview">
                                  <div className="token-id-big">#{offer.requestedNFT}</div>
                                </div>
                              </div>
                            </div>
              ...
              {activeTab === 'browse' && (
                <div className="browse-offers">
                  ...
                  {allOffers.length === 0 ? (
                    ...
                    <div className="offers-list">
                      {allOffers.map(offer => {
                        const canAccept = myNFTs.some(nft => nft.tokenId === offer.requestedNFT);
                        return (
                          <div key={offer.offerId} className="offer-card">
                            <div className="offer-header">
                              <h3>Offre #{offer.offerId}</h3>
                            ...
                            <div className="offer-body">
                              <div className="offer-nft">
                                ...
                                <div className="token-id-big">#{offer.creatorNFT}</div>
                              </div>
                              <div className="exchange-arrow">⇄</div>
                              <div className="offer-nft">
                                ...
                                <div className="token-id-big">
                                  #{offer.requestedNFT}
                                  {canAccept && <span className="badge-own">Vous l'avez !</span>}
                                </div>
                              </div>
                            </div>
                            <div className="offer-actions">
                              ...
                              >
                                {canAccept ? ' Accepter l\'échange' : ' Vous n\'avez pas ce NFT'}
                              </button>
                            </div>
                          </div>
                        );
